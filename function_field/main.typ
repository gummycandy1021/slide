...
=== $A$の特徴
*$A$は整数環$ZZ$と似ている*ことが知られている。

以降のスライドで、その類似点を紹介する。

== 有限体上の多項式環$A$と整数環$ZZ$の類似点

$A$には整数論のもとの同様の方法で定義できる関数や同様の定理が多数ある。
...
#table(
  ...
  [代数体（$QQ$の有限次拡大体）], [大域関数体（$FF_q (T)$の有限次拡大体）]
)
...
また、Bombieriによる、より初等的な証明が院生向けの教科書に載っている。@zbMATH01716471
...
- 関数体は代数体と似ている。
- 関数体の方が証明が簡単な場合がある（関数体バージョンのRiemann予想、Fermatの最終定理、ABC予想は証明済み）。
- それらの証明は院生向けの教科書に載っている。@zbMATH01716471


整数論に興味のある方は、関数体についても学んでみてはいかがでしょうか？

もしかしたら、関数体を調べることでこれらの整数論の難問に対するヒントを得られるかも……？

#figure(
  image("qrcode.png",width: 19%),
  caption: [スライドへのリンク]
)
