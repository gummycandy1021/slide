...
このスライドでは有限体上の1変数有理関数体やその有限次拡大体のこと。
...
#theorem(title: "関数体バージョンのFermatの最終定理")[
  ...
  $X^N + Y^N = 1$の形の方程式をFermat方程式と呼ぶ。
  $N$を$F$の標数$p$を割り切らない数とする。
  - $K$の種数$g_K$が$0$で、$N gt.eq 3$のとき、$K$においてFermat方程式は定数以外の解を持たない。
  - $g_K gt.eq 1$で、$N gt 6g_K - 3$のとき、$K$においてFermat方程式は定数以外の解を持たない。
]
